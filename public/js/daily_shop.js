(() => {
    ...
    const dailyShopElement = (element, vbucks) => {
        return $('<div></div>').addClass('card shadow p-3 mb-5 bg-white').append($('<a></a>').attr({
            ...
            alt: element.name
        }).height(300).width(300))).append($('<div></div>').addClass('card-body').append($('<h5></h5>').addClass('card-title').text(element.name).append($('<h6></h6>').text(element.item.description))).append($('<p></p>').text(`${element.cost} `).addClass('card-text').append($('<img/>').attr({
            src: vbucks,
            height: 25
