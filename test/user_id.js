(() => {
    ...
    const dailyShopElement = (element, vbucks) => {
        return $('<div></div>').addClass('card shadow p-3 mb-5 bg-white').append($('<a></a>').text(element.name).attr({
            ...
            alt: element.name
        }).height(300).width(300)).append($('<div></div>').addClass('card-body').append($('<div></div>').attr({
            id: 'itemdesc'
        }).append($('<h5></h5>').addClass('card-title itemname').text(element.name)).append($('<p></p>').text(`${element.cost} `).addClass('card-text').append($('<img/>').attr({
            src: vbucks,
            height: 25
